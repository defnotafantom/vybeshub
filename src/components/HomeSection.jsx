    ...
    <div className="flex justify-center w-full">
      <div className="w-full max-w-4xl p-4">
        <div className="bg-gray-100/80 backdrop-blur-xl rounded-3xl shadow-2xl p-6 flex flex-col gap-4 border border-blue-200/40 max-h-[80vh] overflow-y-auto relative">
          ...
          <NewPostPopup
            ...
            onPostSubmit={async (postData) => {
              ...
              if (!error) {
                ...
                // Aggiorna UI locale aggiungendo il post in cima
                posts.unshift(data[0]); // oppure setPosts([data[0], ...posts]) se posts è uno state
              } else {
                console.error("Errore pubblicazione post:", error);
              }
            }}
          />
          ...
          {/* Lista post */}
          {posts
            .filter(post => !filterTag || post.art_tag?.split(", ").includes(filterTag))
            .map(post => (
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex flex-col">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                      {post.art_tag?.split(", ").map((tag, idx) => (
                        <span key={idx} className="px-2 py-1 bg-gray-200/80 rounded-xl text-xs">{tag}</span>
                      ))}
                    </div>
                  </div>
                </div>
